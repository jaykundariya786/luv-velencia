...
const ComprehensiveInventory: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<
    "overview" | "alerts" | "forecast" | "locations" | "audits"
  >("overview");
  ...
  const fetchInventoryData = async () => {
    try {
      setLoading(true);
      const [alertsRes, forecastsRes, locationsRes, auditsRes] =
        await Promise.all([
      ...
      const [alertsData, forecastsData, locationsData, auditsData] =
        await Promise.all([
          alertsRes.json(),
          forecastsRes.json(),
          locationsRes.json(),
  ...
  const resolveAlert = async (alertId: string) => {
    try {
      const response = await fetch(
        `/api/admin/inventory/alerts/${alertId}/resolve`,
        {
          method: "PUT",
        }
      );
  ...
  const exportInventoryReport = async (
    ...
    try {
      ...
      if (response.ok) {
        ...
        a.href = url;
        a.download = `inventory-${type}-${format(
          new Date(),
          "yyyy-MM-dd"
        )}.xlsx`;
  ...
  const filteredAlerts = alerts.filter((alert) => {
    ...
    const matchesLocation =
      selectedLocation === "all" || alert.location === selectedLocation;
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col lg:flex-row gap-4 items-center justify-between">
        <div>
          ...
          <p className="text-gray-600">
            Advanced inventory tracking, forecasting, and optimization
          </p>
        </div>
      ...
      <div className="border-b border-gray-200">
        <nav className="-mb-px flex space-x-8">
          {(
            ["overview", "alerts", "forecast", "locations", "audits"] as const
          ).map((tab) => (
            <button
              key={tab}
      ...
      {/* Overview Tab */}
      {activeTab === "overview" && <InventoryManager />}
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-yellow-600">
                    {
                      alerts.filter(
                        (a) => a.severity === "medium" && !a.isResolved
                      ).length
                    }
                  </p>
                </div>
                  ...
                  <p className="text-2xl font-bold text-green-600">
                    {
                      alerts.filter(
                        (a) =>
                          a.isResolved &&
                          new Date(a.createdAt) > subDays(new Date(), 1)
                      ).length
                    }
                  </p>
                </div>
